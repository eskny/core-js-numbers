...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthV1 = Math.sqrt(x1 * x1 + y1 * y1);
  const lengthV2 = Math.sqrt(x2 * x2 + y2 * y2);

  const cosTheta = vectors / (lengthV1 * lengthV2);
...
function getLastDigit(value) {
  const stringFromNum = String(value);
  return Number(stringFromNum[stringFromNum.length - 1]);
}
...
function getSumOfDigits(num) {
  ...
  const arrFromString = stringFromNum.split('');
  let sum = 0;
...
function getFloatOnString(str) {
  ...
  return result === '' || result === '.' ? NaN : Number(result);
}
...
function getSumOfNumbers(x1, x2, x3) {
  const num1 = String(x1);
  const num2 = String(x2);
  const num3 = String(x3);

  const lenthAfterDot1 = num1.includes('.') ? num1.split('.')[1].length : 0;
  const lenthAfterDot2 = num2.includes('.') ? num2.split('.')[1].length : 0;
  const lenthAfterDot3 = num3.includes('.') ? num3.split('.')[1].length : 0;

  const maxLenth = Math.max(lenthAfterDot1, lenthAfterDot2, lenthAfterDot3);

  const sum = x1 + x2 + x3;

  return sum.toFixed(maxLenth);
}
...
function getHypotenuse(a, b) {
  ...
  const normalizedA = a / max;
  const normalizedB = b / max;

  return max * Math.sqrt(normalizedA * normalizedA + normalizedB * normalizedB);
}
